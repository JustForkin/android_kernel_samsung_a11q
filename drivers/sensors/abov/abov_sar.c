	...
	if (this && pDevice) {
		...
		switch (i & 0x03) {
		  case SAR_CLOSE: /* < 20mm */
			...
			#else
			input_report_abs(input_sar, ABS_DISTANCE, 0);
			#endif
		  ...
		  case SAR_FAR: /* >20mm */
			...
			#else
			input_report_abs(input_sar, ABS_DISTANCE, 5);
			#endif
			input_sync(input_sar);
			break;
		  default:
		...
		pabov_platform_data_t pplatData)
{
	...
	/*HS60 code for HS60-354 by zhuqiang at 2019/09/24 start*/
	pcb_abov_conf();
	if (pcb_id_code > 0x90 && pcb_id_code < 0x9A)
...
static int abov_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#else
	__set_bit(EV_ABS, input_sar->evbit);
	input_set_abs_params(input_sar, ABS_DISTANCE, -1, 100, 0, 0);
	#endif
	/*HS60 code for AR-ZQL1695-01000000070/HS60-2078 by zhuqiang at 2019/09/24 end*/
