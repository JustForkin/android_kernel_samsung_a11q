...
/*HS60 code for HS60-356 by zhuqiang at 2019/08/07 start*/
static void swtp_sku_conf(void)
{
    u32 *sku_addr = NULL;
    u32 sku_size = 0;
...
static int swtp_probe(struct platform_device * pdev)
{
    ...
    if (ret) {
        ...
        pr_err(" select swtp_irq_active success\n");
...
static int __init swtp_init(void) {
    ...
    /*HS60 code for HS60-356 by zhuqiang at 2019/08/07 start*/
    swtp_sku_conf();
/*HS50 code for HS50-1588 by zhangsongyun at 2020/09/11 start*/
    if(!(((sku_version >= 0x26) && (sku_version<= 0x2A)) || (sku_version == 0x2E)))
/*HS50 code for HS50-1588 by zhangsongyun at 2020/09/11 end*/
    {
        pr_err(" %s globle don't use swtp\n", __func__);
        return -ENODEV;
    }
    /*HS60 code for HS60-356 by zhuqiang at 2019/08/07 end*/
    ...
    if (ret) {
        ...
        pr_err(" %s register swtp pinctrl driver success\n", __func__);
    }
    /* pinctrl end */
